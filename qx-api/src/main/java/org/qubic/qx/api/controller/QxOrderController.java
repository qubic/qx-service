...
public class QxOrderController {
    ...
    private QxOrderTemplate createResponse(String from, QxAssetOrder order) {
        BigInteger latestTick = qxOrderService.getLatestTick();
        return new QxOrderTemplate(latestTick, from, Qx.ADDRESS, order.getInputType(), new BigInteger(Long.toUnsignedString(order.getAmount())), convertToBase64(order));
    }
