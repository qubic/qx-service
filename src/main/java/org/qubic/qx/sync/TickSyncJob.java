...
public class TickSyncJob {
    ...
    public Flux<Long> sync(long currentTick) {
        ...
        return getLowestAvailableTick()
                .doOnNext(lowest -> this.targetTick = currentTick)
    ...
    public Mono<?> updateAllOrderBooks() {
        return coreService.getCurrentTick()
                .flatMapMany(assetService::retrieveAllCurrentOrderBooks)
                .last();
    }
    ...
    private Mono<Boolean> processTickTransactions(Long tickNumber, List<Transaction> txs) {
        ...
        if (CollectionUtils.isEmpty(txs)) {
            ...
            Mono<?> processTrades = assetService.retrieveCurrentOrderBooks(targetTick, assetInformation)
                    .collect(Collectors.toSet())
            ...
            return processTrades
                    ...
                    // TODO .then(tickRepository.storeTickInfo) - at least we need the timestamp
    ...
    private static Serializable handleQxOrderTransactions(Tuple2<Set<OrderBook>, Set<OrderBook>> tuple, List<Transaction> orderTransactions) {
        Map<String, OrderBook> currentOrderBooks = tuple.getT1().stream().collect(Collectors.toMap(TickSyncJob::orderBookKey, x -> x));
        Map<String, OrderBook> previousOrderBooks = tuple.getT2().stream().collect(Collectors.toMap(TickSyncJob::orderBookKey, x -> x));
